...
//#endif
        ...
        internal void SendCommand(ushort channel, DescribedList command)
        {
            ...
            Trace.WriteLine($"SEND (ch={channel}) {command}");
        }
        ...
        internal int SendCommand(ushort channel, Transfer transfer, bool first, ByteBuffer payload, int reservedBytes)
        {
            ...
            Trace.WriteLine($"SEND (ch={channel}) {transfer} payload {payloadSize}");
        ...
        static void OnHeartBeatTimer(object state)
        {
            ...
            try
            {
                ...
                Trace.WriteLine($"SEND (ch=0) empty");
            }
            catch
            {
                // ignore failures
        ...
        void SendHeader()
        {
            ...
            Trace.WriteLine($"SEND AMQP 0 1.0.0");
        }
        ...
        {
            Trace.WriteLine($"RECV AMQP {header}");
            lock (this.ThisLock)
            {
                if (this.state == State.OpenPipe)
                {
        ...
        internal bool OnFrame(ByteBuffer buffer)
        {
            ...
            try
            {
                ...
                if (buffer.Length > 0)
                {
                    Trace.WriteLine($"RECV (ch={channel}) {command} payload {buffer.Length}");
                }
                else
                {
                    Trace.WriteLine($"RECV (ch={channel}) {command}");
                }
        ...
        {
            Trace.TraceError("Exception occurred: {exception.ToString()}");
        ...
        {
            Trace.TraceError($"I/O: {0}", exception.ToString());
            if (this.state != State.End)
            {
                this.state = State.End;
        ...
        sealed class Pump
        {
            ...
            public void Start()
            {
                Fx.StartThread(new ThreadStart(this.PumpThread));
            }
